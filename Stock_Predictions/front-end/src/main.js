...
async function fetchReport(data) {
  const contents = [
    {
      ...
      parts: [
        {
          text: 'You are a trading guru. Given data on share prices over the past 3 days, write a report of no more than 150 words describing the stocks performance and recommending whether to buy, hold or sell. Use the examples provided between ### to set the style your response.' 
        }
      ]
    },
    {
  ...
  try {
    ...
    const countTokensResponse = await ai.models.countTokens({
      ...
      contents: prompt,
      config: {
        temperature: 1, // --> varía entre 0.1 y 2, un número bajo te da una respuesta menos creativa que una temperatura alta. Con el riesgo que eso supone.
        //presencePenalty: -2,
        //requencyPenalty: -2 //-> varía entre -2 y 2, un número positivo más alto es menos propenso a repetir los mismos tokens.
      }
    })
    ...
    const response = await ai.models.generateContent({
      ...
      contents: contents,
      config: {
        temperature: 1 // --> varía entre 0.1 y 2
      }
    })
